...
async function initSocket(state){
  try {
    state.wsSocket = await new WebSocket("ws://localhost:5000");
    ...
    state.wsSocket.onmessage = async (event) => {
      ...
      if (data.msgType === "callOffer") {
        ...
        state.peerConnection.addStream(state.localStream)
...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    getLocalStream: async (state) => {
      try {
        ...
        if (localStream !== {}) {
          ...
          state.localStream.getAudioTracks()[0].enabled = state.enableAudio;
          state.localStream.getVideoTracks()[0].enabled = state.enableVideo;
        } else {
        }
      } catch (error) {
    ...
    toggleAudio: (state) => {
      ...
      state.localStream.getAudioTracks()[0].enabled = state.enableAudio;
      if (state.enableAudio === true) {
        console.log(state.localStream.getAudioTracks()[0].getConstraints());
        // .applyConstraints(state.defaultConstraints.audio)
      }
    },
    toggleVideo: (state) => {
